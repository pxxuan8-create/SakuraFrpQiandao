...
> 本次签到获得流量 = 签到后共获得流量 − 签到前共获得流量，数据读取自面板"统计信息"弹窗（"共获得流量 X GiB"、"总计签到 N 天"）。
...
面板首页会弹出黑色广告弹窗（"【查看帮助文档】立刻自助解决99%的问题"），且**位置会滚动移动**，可能恰好挡住"点击这里签到"按钮或"统计信息"入口，导致点击失败或读取不到数据。

脚本已内置处理：每次关键操作前都会用 JS 直接移除该弹窗（并兜底点击"关闭"按钮），无需额外配置。若日志出现 `读取统计信息失败`，多半是该弹窗或新弹窗干扰，可反馈弹窗文案到日志再调整移除规则。
